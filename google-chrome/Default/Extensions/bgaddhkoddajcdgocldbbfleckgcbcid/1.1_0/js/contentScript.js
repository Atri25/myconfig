document.addEventListener('DOMContentLoaded', event => {
    ...
    let getCookiesForActiveTab = ((saveAs = false) => {
        chrome.tabs.query({active: true, lastFocusedWindow: true}, tabs => {
    ...
    let getCookiesForActiveTabAndCopy = ((saveAs = false) => {
        chrome.tabs.query({active: true, lastFocusedWindow: true}, tabs => {
            ...
            getCookiesForURL(url).then(data => {
                ...
                navigator.clipboard.writeText(metadata.concat(cookies).concat('\n'))
                    .then(() => {
    ...
    let getCookiesForAllLogins = ((saveAs = false) => {
        chrome.tabs.query({active: true, lastFocusedWindow: true}, tabs => {

            let url = tabs[0].url;
            getAllCookies(url).then(data => {
    ...
    document.getElementById("downloadAs").addEventListener('click', (event) => {
        getCookiesForActiveTab(true);
    });

    document.getElementById("copy").addEventListener('click', (event) => {
        getCookiesForActiveTabAndCopy();
    });

    document.getElementById("downloadAll").addEventListener('click', (event) => {
        getCookiesForAllLogins();
    });
