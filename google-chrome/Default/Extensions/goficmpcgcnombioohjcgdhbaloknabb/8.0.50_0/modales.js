...
function anyadirBotonesYIcono(content, buttons, icon, center) {
  ...
  if (icon) {
    ...
    switch (icon) {
      case 'info': iconText = `<span class="swal2-icon-text">i</span>`;break;
      case 'question': iconText = `<span class="swal2-icon-text">x</span>`;break;
      case 'error': iconText = '<span class="swal2-x-mark"><span class="swal2-x-mark-line-left"></span><span class="swal2-x-mark-line-right"></span></span>';break;
    }
...
function showNewFeature(mens, url) {
  if (localStorage[mens] != "false")  {
    openSnackbar({
      text: chrome.i18n.getMessage(mens),
      action: chrome.i18n.getMessage('noMostrarMas'),
