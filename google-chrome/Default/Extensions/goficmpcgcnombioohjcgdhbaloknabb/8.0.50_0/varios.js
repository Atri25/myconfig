...
function cambiaContador() {
  ...
  switch (localStorage["contador"])
  {
    case '1':
      ...
      break;
    case '2': numPost = localStorage["totalPost"];break;
    case '3': numPost = 0;
  }
  if (numPost > 0)
    chrome.browserAction.setBadgeText({text:String(numPost)});
  else
    chrome.browserAction.setBadgeText({text:''});

  if (localStorage["totalPost"] > 0)
    chrome.browserAction.setTitle({title:chrome.i18n.getMessage("extName") + ' (' + localStorage["totalPost"] + ')'});
  else
...
function deslizaPostVisibles(dir,soloCanvas) {
  var altura = $('.paneles2').innerHeight();
  var margen = 70;
...
function permisoOrigenesImagenes(creaNota, numPost) {
  chrome.permissions.request({origins: ['https://www.noteboardapp.com/']}, function(granted) {
    if (granted)
    {
      abreFormularioFoto(creaNota, numPost);
      console.log('concedido');
    }
    else console.log('no concedido');
  });
...
function clickImgSelec(evt,numPost) {
  ...
  if (imgSelec == 0 || $this.closest('.selectorPins').parent().find('.tipoPins' + imgSelec).length > 0) {
    $selector.slideUp(function(){$(this).remove()});
    return;
  }
  $selector.slideUp(function(){$(this).remove()});
...
function vaciarPapelera(evt,dias) {
  if (!dias) {
    ...
    if (idiomaEsp(navigator.language)) {mens = '¿' + mens;}
...
function moverPostitTablon(numPost, tablon, callback) {
  ...
  $postit.animate({ top: $('#tabPa2' + tablon).offset().top, left: $('#tabPa2' + tablon).offset().left, opacity: "0.2" }, 400, function (kk) {
    $postit.remove();
    if (callback) callback();
    refrescaOrdenacion();
  });
...
function borrarConfirmado(numPost, elimComp, vaciarPapelera) {
  ...
  if (origen == '0') inicializaPapelera();
...
function modificaNotaWeb(datosNota) {
...
function borraNotaWeb(datosNota) {
  var numPost = datosNota.numPost;
...
function adjuntaHtmlWeb(datosNota) {
  var numPost = datosNota.numPost;
...
function compruebaNotasWeb() {
  ...
  if (notasWeb) { // si estamos en el popup miramos si hay permisos aceptados
    if (document.URL.indexOf('popup.html') > 0 || document.URL.indexOf('options.html') > 0) {
      chrome.permissions.contains({permissions: ['tabs'],origins: ["http://*/*","https://*/*"]}, function(granted) {
          if (!granted) {
            sweetAlert({
              text: chrome.i18n.getMessage('notasWebPermisos'),
              successCallback: function () {
                chrome.permissions.request({permissions: ['tabs'],origins: ["http://*/*","https://*/*"]}, function(granted) {
                    if (granted) {
